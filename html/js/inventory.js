...
window.addEventListener("message", function (event) {
    if (event.data.action == "display") {
        ...
        $(".nearbyPlayerButton").click(function () {
            ...
            $.post("http://taeratto_inventory/GiveItem", JSON.stringify({
                player: player,
...
function closeInventory() {
    $.post("http://taeratto_inventory/NUIFocusOff", JSON.stringify({}));
}
...
function inventorySetup(items) {
    ...
    $.each(items, function (index, item) {
        ...
        /*$('#item-' + index).hover(function(){
          ...
          $('#item-' + index).mousedown(function(){
            $('#item-' + index).css({
              'transform' : ' scale(1)',
              'transition' : 'transform .2s'
            });
          });
          $('#item-' + index).mouseup(function(){
            $('#item-' + index).css({
              'transform' : ' scale(1.5)',
              'transition' : 'transform .2s'
            });
...
$(document).ready(function () {
    ...
    $('#use').droppable({
        ...
        drop: function (event, ui) {
            ...
            if (itemData.usable) {
                ...
                $.post("http://taeratto_inventory/UseItem", JSON.stringify({
                    item: itemData
                }));
            }
        }
    ...
    $('#give').droppable({
        ...
        drop: function (event, ui) {
            ...
            if (itemData.canRemove) {
                disableInventory(300);
                $.post("http://taeratto_inventory/GetNearPlayers", JSON.stringify({
                    item: itemData
                }));
            }
        }
    ...
    $('#drop').droppable({
        ...
        drop: function (event, ui) {
            ...
            if (itemData.canRemove) {
                disableInventory(300);
                $.post("http://taeratto_inventory/DropItem", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            }
    ...
    $('#playerInventory').droppable({
        drop: function (event, ui) {
            ...
            if (type === "trunk" && itemInventory === "second") {
                ...
                $.post("http://taeratto_inventory/TakeFromTrunk", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "property" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://taeratto_inventory/TakeFromProperty", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "vault" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://taeratto_inventory/TakeFromVault", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "glovebox" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://taeratto_inventory/TakeFromGlovebox", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "player" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://taeratto_inventory/TakeFromPlayer", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "shop" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://taeratto_inventory/TakeFromShop", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val()),
    ...
    $('#otherInventory').droppable({
        drop: function (event, ui) {
            ...
            if (type === "trunk" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://taeratto_inventory/PutIntoTrunk", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "property" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://taeratto_inventory/PutIntoProperty", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "vault" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://taeratto_inventory/PutIntoVault", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "glovebox" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://taeratto_inventory/PutIntoGlovebox", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "player" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://taeratto_inventory/PutIntoPlayer", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            }
